...
export default function DonatePage() {
    ...
    useEffect(() => {
        async function fetchRequests() {
            ...
            if (result.success) {
                ...
                toast("Error", {
                    description: result.error || "Failed to fetch donation requests.",
                })
            }
            setIsLoading(false)
        }
    ...
    const handleStatusUpdate = (id: string, status: string) => {
        startTransition(async () => {
            const result = await updateDonationRequestStatus(id, status)
            if (result.success) {
    ...
    if (isLoading) {
        return (
            <div className="container mx-auto py-8 px-4">
                <div className="text-center py-12">Loading donation requests...</div>
            </div>
        )
    }
